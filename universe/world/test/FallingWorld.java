 ...
 *                                                                       *
 *   FILE: ./universe/world/test/FallingWorld.java                       *
 *                                                                       *
...
public class FallingWorld extends World<None>{
    ...
    public Package<None> onMouse(int x, int y, String me){
        if(!me.equals("drag"))return new Package<None>(this);
        return new Package<None>(
                new FallingWorld(new CLoB(this.rand.randBall(x,y), this.balls)));
    }
    /** On each tick, we apply gravity, move them, and filter out
     *    those off screen */
    public Package<None> onTick(){
        return new Package<None>(
                new FallingWorld(this.balls.gravity().move().filter()));    
    }
}
